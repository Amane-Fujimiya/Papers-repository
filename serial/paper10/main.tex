...
\title{ The Hall effects in electromagnetic-present, quantum well structure of electron-photon scattering}

%Theoretical study of the Hall effect in infinite semi-parabolic Quantum Wells in the present of electromagnetic wave in optical electron-phonon scattering by using quantum kinetic equation
...
\section{The specific energy and wavefunctional}

Our setting throughout this paper is concerned of the \textbf{energy spectrum} and \textbf{wavefunction} of electrons in a quantum well when placed in perpendicular electric and magnetic fields. Consider an electron trapped in a quantum well of the form:
\begin{equation}
    V(z) =
    \begin{cases}
        \infty, & z<0\\
...
\end{equation}
where N is the Landau level index and n is the sublevel index.; $\omega_c=eB/m_{e}$ is cyclotron frequency; $\nu_d= -E_1 /B$ is the velocity of electron drift; $\Phi(x-x_0)$ is the wave function centered at $x_0 = - l^2 (k_y-(m_e \nu_d)/\hbar)$, where $l=\sqrt{\hbar/eB}$ is the radius of the cyclotron in the $Oxy$ plane; and $\phi_n(z)$ is an eigenfunction and is given by:
\begin{equation}
    \phi_n(z)=A_n  \cdot \exp{\left(-\frac{z^2}{2\alpha_z^2}\right)} \cdot H_{2n+1}(\frac{z}{\alpha_z}), \quad \alpha_z=\sqrt{\frac{\hbar}{m_e \omega_z}}
\end{equation}
where $H$ is the Hermite polynomials.

\section{The asymmetric, semi-parabolic quantum well}

In this particular paper, we focus to resolve the Hall effect in the above setting, for \textbf{finite additive asymmetric semi-parabolic quantum well}, specifically with electron-optical phonon scattering under the influence of a strong electromagnetic wave. In all, the method of solution use the theory of quantum kinetic as the main technique for the following sections. The Hamiltonian of the electron-phonon network in the second quantized representation is written as
\begin{equation}
\begin{aligned}
...
Using Hamiltonian \eqref{H} in the quantum dynamic equations for the particle number operator in the finite additive asymmetric semi-parabolic quantum well:
\begin{equation}
    i \hbar \frac{\partial f_{N,n,\vec{k_y}}(t)}{\partial t} = \langle [a_{N,n,\vec{k_y}}^+ a_{N,n,\vec{k_y}},H] \rangle_t
\end{equation}
...
When the temperature increases from 200 K to 400 K, the Hall coefficient decreases steadily in magnitude, especially clearly at strong magnetic field ($B=12T$). This is consistent with the fact that as the temperature increases, the number of phonons increases, leading to increased scattering and decreased mobility of charge carriers, which reduces the Hall effect.
...
\bibitem{P}
P. Vasilopulos, Charbonneau M., and Van Vliet C.M(1987), "Linear and nonlinear electrical conduction in quasi-two-dimensional quantum wells", \textit{Phy.Rev.B} (33), 1334-1344
